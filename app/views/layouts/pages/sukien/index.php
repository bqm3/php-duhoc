<?php
// app/views/layouts/pages/sukien/index.php
$base_url = $base ?? '';
?>
...
<div class="container py-5">
    <?php if (!empty($eventPosts)): ?>
        <div class="row g-4 justify-content-center">
            <?php foreach ($eventPosts as $post): ?>
                <div class="col-12">
                    <?php partial('layouts/pages/sukien/event_card', ['post' => $post, 'base' => $base]); ?>
                </div>
            <?php endforeach; ?>
        </div>
    ...
    <?php else: ?>
        <div class="text-center py-5 shadow-sm rounded-4 bg-light">
            ...
            <h3 class="mb-3">Chưa có sự kiện nào</h3>
            <p class="text-muted mb-0">Chúng tôi sẽ sớm cập nhật các sự kiện mới nhất.</p>
        </div>
    <?php endif; ?>
</div>

<style>
    /* Reuse Tuyển dụng styles for Sự kiện */
    .career-card {
        background: #fff;
        border-radius: 12px;
        overflow: hidden;
