...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    ...
    function renderMenu(res) {
      ...
      const continents = res.items || [];
      if (continents.length === 0) {
        menuContent.innerHTML = '<div class="col-12 text-center">Chưa có bài viết du học</div>';
        return;
      }
      ...
      <div class="col-md-4 mb-3">
        <h6 class="dropdown-header px-0 text-primary fw-bold">${escapeHtml(ct.name)}</h6>
        ${(ct.countries || []).map(x => `
          <a class="dropdown-item px-0 py-1" href="<?= $base ?>${escapeAttr(x.post?.href || '#')}">
            ${escapeHtml(x.name)}
          </a>
        `).join('')}
      </div>
    ...
    function escapeAttr(str) {
      // attr escape tối thiểu để nhét vào href (đã prefix base)
